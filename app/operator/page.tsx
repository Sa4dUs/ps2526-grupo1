...
export default function OperatorPage() {
	...
	const loadProblem = (lvl: number) => {
		...
		const tokens = (
			formattedProblem.question.match(/\d+|[()+\-*/]/g) || []
		).map((value, index) => ({
	...
	const checkAnswer = () => {
		if (!problem) return;
		const submittedEquation = equationSlots
			.map((t) => t?.value || "")
			.join("");
	...
	const handleDragEnd = (event: DragEndEvent) => {
		...
		const sourceSlotIdx = equationSlots.findIndex(
			(t) => t?.id === active.id
		);
					...
					<Card className="w-full max-w-3xl shadow-lg">
						<CardHeader>
							<h2 className="text-center font-bold">
								Question {numberOfQuestions} - Level {level}
							</h2>
						</CardHeader>
